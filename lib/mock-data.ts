...
export type PhaseKind = 'base' | 'build' | 'peak' | 'taper' | 'race' | 'transition'
...
> = {
  sprint: { label: 'Sprint', short: 'Sprint', swim: 750, bike: 20000, run: 5000 },
  olympic: { label: 'Olympic', short: 'Olympic', swim: 1500, bike: 40000, run: 10000 },
  half: { label: 'Half Iron (70.3)', short: '70.3', swim: 1900, bike: 90000, run: 21100 },
  full: { label: 'Full Iron', short: 'IM', swim: 3800, bike: 180000, run: 42200 },
}
...
> = {
  ...
  strength: { label: 'Strength', icon: 'dumbbell', color: 'var(--strength)', short: 'ST' },
  rest: { label: 'Rest', icon: 'moon', color: 'var(--rest)', short: 'RE' },
}

export const intensityMeta: Record<Intensity, { label: string; zone: ZoneKey }> = {
...
> = {
  1: { label: 'Z1 Recovery', short: 'Z1', color: 'var(--zone-1)', desc: 'Active recovery' },
  2: { label: 'Z2 Endurance', short: 'Z2', color: 'var(--zone-2)', desc: 'Aerobic base' },
  3: { label: 'Z3 Tempo', short: 'Z3', color: 'var(--zone-3)', desc: 'Steady tempo' },
  4: { label: 'Z4 Threshold', short: 'Z4', color: 'var(--zone-4)', desc: 'Lactate threshold' },
  5: { label: 'Z5 VO2', short: 'Z5', color: 'var(--zone-5)', desc: 'VO2 max' },
  6: { label: 'Z6 Anaerobic', short: 'Z6', color: 'var(--zone-6)', desc: 'Anaerobic capacity' },
  7: { label: 'Z7 Neuro', short: 'Z7', color: 'var(--zone-7)', desc: 'Neuromuscular' },
}
...
export const goals: Goal[] = [
  {
    ...
    targetTime: '4:25:00',
    progress: 68,
    notes: 'Season A-race. Target 5:05/km run off the bike, 21.5 min/40k FTP hold.',
  },
  {
    id: 'g2',
...
function buildMetrics(): DayMetric[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    let tss = 0
    if (dow === 6) tss = 0 // rest day
    else if (dow === 2) tss = base * 0.55 * (0.9 + rand() * 0.2)
    else if (dow === 5) tss = base * 1.8 * (0.9 + rand() * 0.2) // long day
...
const templates: Record<string, Template> = {
  swimTechnique: {
    ...
    intervals: [
      { label: 'Warm-up', durationSec: 600, zone: 1, targetLow: 60, targetHigh: 70 },
      { label: 'Drills 8×50', durationSec: 720, zone: 2, targetLow: 70, targetHigh: 80 },
      { label: 'Main 6×200', durationSec: 1440, zone: 3, targetLow: 88, targetHigh: 95 },
      { label: 'Cool-down', durationSec: 300, zone: 1, targetLow: 55, targetHigh: 65 },
    ],
  },
  swimThreshold: {
    sport: 'swim',
    ...
    intervals: [
      { label: 'Warm-up', durationSec: 720, zone: 1, targetLow: 60, targetHigh: 70 },
      { label: '10×100 CSS', durationSec: 1500, zone: 4, targetLow: 98, targetHigh: 103 },
      { label: 'Pull 400', durationSec: 600, zone: 2, targetLow: 75, targetHigh: 82 },
      { label: 'Cool-down', durationSec: 360, zone: 1, targetLow: 55, targetHigh: 65 },
    ],
  },
  bikeEndurance: {
    sport: 'bike',
    ...
    intervals: [
      { label: 'Warm-up', durationSec: 600, zone: 1, targetLow: 50, targetHigh: 60 },
      { label: 'Steady Z2', durationSec: 3600, zone: 2, targetLow: 65, targetHigh: 75 },
      { label: 'Tempo surges 3×5', durationSec: 900, zone: 3, targetLow: 84, targetHigh: 90 },
      { label: 'Steady Z2', durationSec: 1800, zone: 2, targetLow: 65, targetHigh: 74 },
      { label: 'Cool-down', durationSec: 300, zone: 1, targetLow: 45, targetHigh: 55 },
    ],
  },
  bikeThreshold: {
    sport: 'bike',
    ...
    intervals: [
      { label: 'Warm-up', durationSec: 1200, zone: 1, targetLow: 50, targetHigh: 65 },
      { label: 'Set 1 O/U', durationSec: 900, zone: 4, targetLow: 95, targetHigh: 105 },
      { label: 'Recover', durationSec: 300, zone: 1, targetLow: 50, targetHigh: 58 },
      { label: 'Set 2 O/U', durationSec: 900, zone: 4, targetLow: 95, targetHigh: 105 },
      { label: 'Recover', durationSec: 300, zone: 1, targetLow: 50, targetHigh: 58 },
      { label: 'Set 3 O/U', durationSec: 900, zone: 4, targetLow: 95, targetHigh: 106 },
      { label: 'Cool-down', durationSec: 600, zone: 1, targetLow: 45, targetHigh: 55 },
    ],
  },
  bikeVo2: {
    sport: 'bike',
    ...
    intervals: [
      { label: 'Warm-up', durationSec: 1200, zone: 1, targetLow: 50, targetHigh: 65 },
      { label: '5×4 VO2', durationSec: 2400, zone: 5, targetLow: 113, targetHigh: 122 },
      { label: 'Recoveries', durationSec: 1200, zone: 1, targetLow: 50, targetHigh: 58 },
      { label: 'Cool-down', durationSec: 480, zone: 1, targetLow: 45, targetHigh: 55 },
    ],
  },
  runEasy: {
    sport: 'run',
    ...
    intervals: [
      { label: 'Easy Z2', durationSec: 2700, zone: 2, targetLow: 70, targetHigh: 78 },
      { label: 'Strides 4×20s', durationSec: 300, zone: 5, targetLow: 110, targetHigh: 125 },
    ],
  },
  runThreshold: {
    sport: 'run',
    ...
    intervals: [
      { label: 'Warm-up', durationSec: 900, zone: 1, targetLow: 65, targetHigh: 75 },
      { label: '4×6 Threshold', durationSec: 1440, zone: 4, targetLow: 97, targetHigh: 103 },
      { label: 'Jog recoveries', durationSec: 360, zone: 1, targetLow: 60, targetHigh: 68 },
      { label: 'Cool-down', durationSec: 600, zone: 1, targetLow: 62, targetHigh: 70 },
    ],
  },
  brick: {
    sport: 'bike',
    ...
    rationale: 'Rehearse race intensity and transitions; train run legs fatigued.',
    intervals: [
      { label: 'Bike warm-up', durationSec: 900, zone: 1, targetLow: 50, targetHigh: 62 },
      { label: 'Bike race pace 3×15', durationSec: 2700, zone: 3, targetLow: 82, targetHigh: 90 },
      { label: 'Bike steady', durationSec: 2400, zone: 2, targetLow: 68, targetHigh: 76 },
      { label: 'Run off bike', durationSec: 1800, zone: 3, targetLow: 85, targetHigh: 92 },
    ],
  },
  longRun: {
    sport: 'run',
    ...
    intervals: [
      { label: 'Steady Z2', durationSec: 4800, zone: 2, targetLow: 70, targetHigh: 78 },
      { label: 'Finish MP', durationSec: 1200, zone: 3, targetLow: 84, targetHigh: 90 },
    ],
  },
  strength: {
    sport: 'strength',
    ...
    intervals: [
      { label: 'Mobility', durationSec: 600, zone: 1, targetLow: 40, targetHigh: 55 },
      { label: 'Main lifts', durationSec: 1500, zone: 2, targetLow: 60, targetHigh: 75 },
      { label: 'Core circuit', durationSec: 600, zone: 3, targetLow: 70, targetHigh: 85 },
    ],
  },
  rest: {
    sport: 'rest',
    ...
    keys.forEach((key, idx) => {
      ...
      if (t.sport === 'rest') {
        ...
        if (roll > 0.9) {
          ...
          const mainZone = t.zoneWeights.indexOf(Math.max(...t.zoneWeights)) + 1
          completed = {
            durationMin,
            tss,
            distance: Math.round(t.dist * comp),
            rpe: Math.min(10, Math.round((intensityMeta[t.intensity].zone + 2) * (0.9 + rand() * 0.2))),
            feeling: (['great', 'good', 'good', 'ok', 'tired'] as const)[Math.floor(rand() * 5)],
            compliance: Math.round(comp * 100),
            avgHr:
              t.sport === 'strength'
                ? undefined
                : Math.round(athlete.thresholdHr * (0.82 + intensityMeta[t.intensity].zone * 0.03)),
            maxHr:
              t.sport === 'strength'
                ? undefined
            ...
            avgPower:
              t.sport === 'bike'
                ? Math.round(athlete.ftp * (0.62 + intensityMeta[t.intensity].zone * 0.06))
                : undefined,
            normPower:
              t.sport === 'bike'
                ? Math.round(athlete.ftp * (0.68 + intensityMeta[t.intensity].zone * 0.06))
                : undefined,
            avgPaceSecPerKm:
              t.sport === 'run'
                ? paceFromZone(athlete.thresholdPaceSecPerKm, mainZone as ZoneKey)
                : undefined,
            avgSpeedKmh:
              t.sport === 'bike'
